...
<template>
  <li class="flex justify-between items-center h-11">
    <input v-if="showInput" class="outline-0 border-none bg-[var(--subtle-color)] p-2 w-full me-2"
      placeholder="Editar categoría" v-model="text" ref="inputRef" />
    <label v-if="!showInput" class="w-full flex items-center cursor-pointer">
      <input type="checkbox" class="me-1 cursor-pointer w-4 h-4" v-model="completed" @change="handleCompleteTask" />
      <div class="truncate">
        {{ text }}
      </div>
    </label>
